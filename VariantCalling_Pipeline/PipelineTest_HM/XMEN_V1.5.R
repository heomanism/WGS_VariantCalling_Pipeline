...
#################################################################################
#################### 3.Alignment_BWA & Converting & Sorting  ####################
#################################################################################
XMEN_BWA_Samtools_Conv_Sort <- function(Sorting_directory,RealThreads,id,PutYourReferenceFile,PutYourTrimmoOutputDir_Pair,id_left_output_paired,id_right_output_paired,Mapping_directory,PutYourReferenceIndexingFile){
  ...
  BWA_Conv_Sort_CMD <- paste0("/data/bin/bwa mem -t ",RealThreads," -R '@RG\\tID:Korea1k\\tPL:ILLUMINA\\tSM:",id,"\\tLB:WonLab' ",PutYourReferenceFile," ",PutYourTrimmoOutputDir_Pair,"/",id_left_output_paired," ",PutYourTrimmoOutputDir_Pair,"/",id_right_output_paired," 2> ",Mapping_directory,"/",id,".log | /data/bin/samtools sort -m ",RealMemory%/%2,"M -@ ",RealThreads," -o ",Sorting_directory,"/",id,".Sorted.bam")
  system(BWA_Conv_Sort_CMD)
}
#################################################################################
############################ 4.Indexing_ Samtools ###############################
#################################################################################
...
#################################################################################
##################### 5.MarkDuplicates_Sortsam_ PICARD ##########################
#################################################################################
XMEN_MarkDup <- function(Markdup_SortSam_directory,Sorting_directory,id){
  ...
  Memory<- as.numeric(Memory)/2
  RealMemory<- Memory%/%fastq_file_length
  Indexing_CMD <- paste0("/data/bin/gatk --java-options '-Xmx",RealMemory*2,"m -Xms",RealMemory%/%2,"m -XX:+UseParallelGC -XX:ParallelGCThreads='",RealThreads," MarkDuplicates -I ",Sorting_directory,"/",id,".Sorted.bam -O ",Markdup_SortSam_directory,"/",id,".dedup.bam -M ",Markdup_SortSam_directory,"/",id,".markdup.metrics.txt")
  system(Indexing_CMD)
}
...
XMEN_SortSam <- function(Markdup_SortSam_directory,id,RealThreads){
  ...
  Memory<- as.numeric(Memory)/2
  RealMemory<- Memory%/%fastq_file_length
  SortSam_CMD <- paste0("/data/bin/gatk --java-options '-Xmx",RealMemory*2,"m -Xms",RealMemory%/%2,"m -XX:+UseParallelGC -XX:ParallelGCThreads='",RealThreads," SortSam -I ",Markdup_SortSam_directory,"/",id,".dedup.bam -O ",Markdup_SortSam_directory,"/",id,".sortsam.bam -SO coordinate && /data/bin/samtools index ",Markdup_SortSam_directory,"/",id,".sortsam.bam -@ ",RealThreads)
  system(SortSam_CMD)
}

#################################################################################
########################## 6.BQSR_ApplyBQSR_ GATK ###############################
#################################################################################
...
XMEN_HaplotypeCaller <- function(VaraintCalling_directory,PutYourReferenceFile,Markdup_SortSam_directory,id,RealThreads){
  ...
  Memory<- as.numeric(Memory)/2
  RealMemory<- Memory%/%fastq_file_length
  HaplotypeCaller_CMD <- paste0("/data/bin/gatk --java-options '-Xmx",RealMemory*2,"m -Xms",RealMemory%/%2,"m' HaplotypeCaller -R ",PutYourReferenceFile," -I ",BQSR_directory,"/",id,".final.bam -O ",VaraintCalling_directory,"/",id,".vcf -ERC GVCF --output-mode EMIT_VARIANTS_ONLY -ploidy 2 --stand-call-conf 2 --native-pair-hmm-threads ",RealThreads)
  system(HaplotypeCaller_CMD)
}

#################################################################################
######################## 8.Making Thread_function ###############################
#################################################################################
...
#### Make Directory 
MKdir <- c()
MKdir[1] <- paste0("mkdir ",VCF_pipeline_Directory,"/1.Trimmming_Trimmomatic")
MKdir[2] <- paste0("mkdir ",VCF_pipeline_Directory,"/2.QC_FastQC")
MKdir[3] <- paste0("mkdir ",VCF_pipeline_Directory,"/3.Alignment_BWA")
MKdir[4] <- paste0("mkdir ",VCF_pipeline_Directory,"/4.Converting_Sorting_Samtools")
MKdir[5] <- paste0("mkdir ",VCF_pipeline_Directory,"/5.BAM_Indexing_Samtools")
MKdir[6] <- paste0("mkdir ",VCF_pipeline_Directory,"/6.MarkDup_SortSam_GATK")
MKdir[7] <- paste0("mkdir ",VCF_pipeline_Directory,"/7.BQSR_GATK")
MKdir[8] <- paste0("mkdir ",VCF_pipeline_Directory,"/8.VariantCalling_GATK")
MKdir[9] <- paste0("mkdir ",VCF_pipeline_Directory,"/1.Trimmming_Trimmomatic/Paired")
MKdir[10] <- paste0("mkdir ",VCF_pipeline_Directory,"/1.Trimmming_Trimmomatic/Unpaired")
...
for(i in 1:5){
  system(paste0("echo 'You have ",fastq_file_length*2," 2 fastq files per 1 sample '"))
}
...
for(i in 1:fastq_file_length){
  ...
  id_left[i] <- paste0(id[i],"_R1") ## Left
  id_right[i] <- paste0(id[i],"_R2") ## Right
  
  Full_id_left[i] <- paste0(id[i],"_R1.clean.fq.gz") ## Left
  Full_id_right[i] <- paste0(id[i],"_R2.clean.fq.gz") ## Right
...
#### Multi-core (threads) Parallel Setting
...
#### Making Trcae
system(paste0("touch ",VCF_pipeline_Directory,"/1.Trimmming_Trimmomatic/Trimmomatic_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/2.QC_FastQC/FastQC_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/4.Converting_Sorting_Samtools/Mapping_Converting_Sorting_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/5.BAM_Indexing_Samtools/BAM_Indexing_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/6.MarkDup_SortSam_GATK/MarkDup_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/6.MarkDup_SortSam_GATK/SortSam_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/7.BQSR_GATK/BQSR_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/8.VariantCalling_GATK/VariantCalling_Trace.txt"))
system(paste0("touch ",VCF_pipeline_Directory,"/Completed_Sample.txt"))
...
foreach(i=1:fastq_file_length,.packages = c("tidyverse","dplyr")) %dopar% {
  ...
  if(length(list.files(path=PutYourTrimmoOutputDir_Pair,pattern=paste0(id_left[i],".paired.fq.gz")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip Trimming process'"))
  }else{
    if(length(list.files(path=PutYoutFastqDir,pattern=paste0(id_left[i],".clean.fq.gz")))!=0 && length(PutYourTrimAdapter_Dir_ID)!= 0){
      system(paste0("echo ",id[i]," 'Start Trimmomatic'"))
      system(paste0("echo ",id[i]," 'Start Trimmomatic' >>",VCF_pipeline_Directory,"/1.Trimmming_Trimmomatic/Trimmomatic_Trace.txt")) ## Trace
      ...
      system(paste0("echo ",id[i]," 'Error in your inputs for processing Trimmomatic'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to trimming'"))
      stop()
    }
  }
  ## Fastqc (QC checking)
  if(length(list.files(path=QC_directory,pattern=paste0(id_left[i],".paired_fastqc.html")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip fastQC process'"))
  }else{
    if(length(list.files(path=PutYourTrimmoOutputDir_Pair,pattern=paste0(id_left[i],".paired.fq.gz")))!=0){
      system(paste0("echo ",id[i]," 'Start fastQC'"))
      system(paste0("echo ",id[i]," 'Start fastQC' >>",VCF_pipeline_Directory,"/2.QC_FastQC/FastQC_Trace.txt"))
      XMEN_fastQC(QC_directory,RealThreads,PutYourTrimmoOutputDir_Pair,id_left_output_paired[i],id_right_output_paired[i],id[i])
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing fastQC'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to fastQC'"))
      stop()
    }
  }
  ## BWA & Samtools (Converting & Sorting)
  if(length(list.files(path=Sorting_directory,pattern=paste0(id[i],".Sorted.bam")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip Mapping & Convertinng & Sorting process'"))
  }else{
    if(length(list.files(path=PutYourTrimmoOutputDir_Pair,pattern=paste0(id_left[i],".paired.fq.gz")))!=0){
      system(paste0("echo ",id[i]," 'Start Mapping & Converting & Sorting'"))
      system(paste0("echo ",id[i]," 'Start Mapping & Converting & Sorting' >>",VCF_pipeline_Directory,"/4.Converting_Sorting_Samtools/Mapping_Converting_Sorting_Trace.txt"))
      XMEN_BWA_Samtools_Conv_Sort(Sorting_directory,RealThreads,id[i],PutYourReferenceFile,PutYourTrimmoOutputDir_Pair,id_left_output_paired,id_right_output_paired,Mapping_directory,PutYourReferenceIndexingFile)
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing Converting & Sorting'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to Converting & Sorting'"))
      stop()
    }
  }
  ## Samtools Indexing (BAM file Indexing)
  if(length(list.files(path=Indexing_directory,pattern=paste0(id[i],".index")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip BAM file Indexing process'"))
  }else{
    if(length(list.files(path=Sorting_directory,pattern=paste0(id[i],".Sorted.bam")))!=0){
      system(paste0("echo ",id[i]," 'Start BAM Indexing'"))
      system(paste0("echo ",id[i]," 'Start BAM Indexing' >>",VCF_pipeline_Directory,"/5.BAM_Indexing_Samtools/BAM_Indexing_Trace.txt"))
      XMEN_Samtools_Indexing(Indexing_directory,Sorting_directory,id[i],RealThreads)
    }else{
      system(paste0("echo ",id[i],"'Error in your inputs for processing BAM file Indexing'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to BAM file Indexing'"))
      stop()
    }
  }
  ...
  if(length(list.files(path=Markdup_SortSam_directory,pattern=paste0(id[i],".dedup.bam")))!=0 && length(list.files(path=Markdup_SortSam_directory,pattern=".markdup.metrics.txt"))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip MarkDuplicates process'"))
  }else{
    if(length(list.files(path=Sorting_directory,pattern=paste0(id[i],".Sorted.bam")))!=0){
      system(paste0("echo ",id[i]," 'Start MarkDuplicates'"))
      system(paste0("echo ",id[i]," 'Start MarkDuplicates' >>",VCF_pipeline_Directory,"/6.MarkDup_SortSam_GATK/MarkDup_Trace.txt"))
      XMEN_MarkDup(Markdup_SortSam_directory,Sorting_directory,id[i])
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing MarkDuplicates'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to MarkDuplicates'"))
      stop()
    }
  }
  ...
  if(length(list.files(path=Markdup_SortSam_directory,pattern=paste0(id[i],".sortsam.bam")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip SortSam process'"))
  }else{
    if(length(list.files(path=Markdup_SortSam_directory,pattern=paste0(id[i],".dedup.bam")))!=0 && length(list.files(path=Markdup_SortSam_directory,pattern=".markdup.metrics.txt"))!=0){
      system(paste0("echo ",id[i]," 'Start Sortsam'"))
      system(paste0("echo ",id[i]," 'Start Sortsam' >>",VCF_pipeline_Directory,"/6.MarkDup_SortSam_GATK/SortSam_Trace.txt"))
      XMEN_SortSam(Markdup_SortSam_directory,id[i],RealThreads)
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing SortSam'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to SortSam'"))
      stop()
    }
  }
  ...
  if(length(list.files(path=BQSR_directory,pattern=paste0(id[i],".final.bam")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip BQSR process'"))
  }else{
    if(length(list.files(path=Markdup_SortSam_directory,pattern=paste0(id[i],".sortsam.bam")))!=0 && length(PutYourReferenceFile)!=0 && length(PutYourKnownSitesVCF)!=0){
      system(paste0("echo ",id[i]," 'Start BQSR'"))
      system(paste0("echo ",id[i]," 'Start BQSR' >>",VCF_pipeline_Directory,"/7.BQSR_GATK/BQSR_Trace.txt"))
      XMEN_BQSR(BQSR_directory,Markdup_SortSam_directory,id[i],PutYourReferenceFile,PutYourKnownSitesVCF)
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing BQSR'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to BQSR'"))
      stop()
    }
  }
  ## GATK HaplotypeCaller (BAM -> gVCF)
  if(length(list.files(path=VaraintCalling_directory,pattern=paste0(id[i],".vcf")))!=0){
    system(paste0("echo ",id[i]," 'Data is existed ... Skip HaplotypeCaller process'"))
  }else{
    if(length(list.files(path=BQSR_directory,pattern=paste0(id[i],".final.bam")))!=0 && length(PutYourReferenceFile)!=0){
      system(paste0("echo ",id[i]," 'Start HaplotypeCaller'"))
      system(paste0("echo ",id[i]," 'Start HaplotypeCaller' >>",VCF_pipeline_Directory,"/8.VariantCalling_GATK/VariantCalling_Trace.txt"))
      XMEN_HaplotypeCaller(VaraintCalling_directory,PutYourReferenceFile,Markdup_SortSam_directory,id[i],RealThreads)
      system(paste0("echo ",id[i]," 'Completed' >>",VCF_pipeline_Directory,"/Completed_Sample.txt"))
      if(i==fastq_file_length){
        system(paste0("echo ",id[i]," ' processes are over'"))
      }
    }else{
      system(paste0("echo ",id[i]," 'Error in your inputs for processing HaplotypeCaller'"))
      system(paste0("echo ",id[i]," 'Please check your inputs & directories & files related to HaplotypeCaller'"))
      stop()
    }
  }
}
