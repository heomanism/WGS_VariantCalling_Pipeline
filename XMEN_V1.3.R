...
XMEN_MarkDup <- function(Markdup_SortSam_directory,Sorting_directory,id){
  ...
  Indexing_CMD <- paste0("/data/bin/gatk --java-options '-Xmx",RealMemory*2,"m -Xms",RealMemory,"m -XX:+UseParallelGC -XX:ParallelGCThreads='",RealThreads," MarkDuplicates -I ",Sorting_directory,"/",id,".Sorted.bam -O ",Markdup_SortSam_directory,"/",id,".dedup.bam -M ",Markdup_SortSam_directory,"/",id,".markdup.metrics.txt")
  system(Indexing_CMD)
}
...
for(i in 1:5){
  system(paste0("echo 'You have ",fastq_file_length*2," 2 per 1 sample '"))
}
...
foreach(i=1:fastq_file_length,.packages = c("tidyverse","dplyr")) %dopar% {
  ...
  if(length(list.files(path=Sorting_directory,pattern=paste0(id[i],".Sorted.bam")))!=0){
    system("echo 'Data is existed ... Skip Mapping & Convertinng & Sorting process'")
  }else{
    if(length(list.files(path=PutYourTrimmoOutputDir_Pair,pattern=paste0(id_left[i],".paired.fq.gz")))!=0){
      system("echo 'Start Mapping & Converting & Sorting'")
